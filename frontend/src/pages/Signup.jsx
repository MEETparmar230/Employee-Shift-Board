     ...
     <div className='text-zinc-200 max-w-xl flex justify-center mx-auto mt-50 shadow'>
       <form onSubmit={handleSubmit} action="" className='border rounded-md p-5 w-full  items-center'>
         ...
         <div className='my-4 mx-auto w-fit text-xl'>
           ...
           <input className='border ms-10 rounded px-2' id='name' type="text" value={formData.name} onChange={e=>setFormData({...formData,name:e.target.value.trim()})} required/>
         </div>
         <div className='my-4 mx-auto w-fit text-xl'>
           <label className='font-medium'  htmlFor="email">Email :</label>
           <input className='border ms-11 rounded px-2 ' id='email' type="text" value={formData.email} onChange={e=>setFormData({...formData,email:e.target.value.trim()})} required/>
         </div>
         <div className='mx-auto w-fit text-xl'>
           <label className='font-medium'  htmlFor="password">Password :</label>
           <input className='border ms-2 rounded px-2' id='password' type="password" value={formData.password} onChange={(e) => setFormData({...formData,password:e.target.value.trim()})} required/>
         </div>
